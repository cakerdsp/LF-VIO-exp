...
bool Estimator::initialStructure()
{
    ...
    for (int i = 0; frame_it != all_image_frame.end(); frame_it++)
    {
        ...
        // changed by wz
        vector<Eigen::Vector3d> pts_2_vector;
        ...
        for (auto &id_pts : frame_it->second.points)
        {
            ...
            for (auto &i_p : id_pts.second)
            {
                ...
                if (it != sfm_tracked_points.end())
                {
                    ...
                    pts_2_vector.push_back(img_pts);
                    Vector3d world_pts = it->second;
                    pts_3_vector.push_back(world_pts);
                }
            }
        }
        // changed by wz
        ...
        int featureNums = pts_3_vector.size();
        pnp.set_maximum_number_of_correspondences(featureNums);
        for (auto num = 0; num < pts_3_vector.size(); num++)
        {
            pnp.add_correspondence(pts_3_vector[num], pts_2_vector[num]);
        }
        Matrix3d R_pnp, tmp_R_pnp;
        Vector3d T_pnp;
        pnp.compute_pose(tmp_R_pnp, T_pnp);
...
void Estimator::optimization()
{
    ...
    for (auto &it_per_id : f_manager.feature)
    {
        ...
        for (auto &it_per_frame : it_per_id.feature_per_frame)
        {
            ...
            int track_size = it_per_id.feature_per_frame.size();

            if (ESTIMATE_TD)
            {
                ProjectionTdFactor *f_td = new ProjectionTdFactor(pts_i, pts_j, it_per_id.feature_per_frame[0].velocity, it_per_frame.velocity,
                                                                  it_per_id.feature_per_frame[0].cur_td, it_per_frame.cur_td,
                                                                  it_per_id.feature_per_frame[0].uv.y(), it_per_frame.uv.y(), track_size);
